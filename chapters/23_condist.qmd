...
The conditional PMF of $Y$ given $X=x$ is defined as $$f_{Y|X} (y|x) = P(Y=y | X=x)=\frac{P(Y=y,X=x)}{P(X=x)}$$ for any $x$ such that $P(X=x)>0$.
:::
...
# compare distributions in the same plot
densityplot(~ final, groups = major, data = exam, 
...
The conditional expectation of $Y$ given $X=x$ is defined as $$\psi(x) = E(Y|X=x)= \sum_{\text{all }y} y\ f_{Y|X}(y|x).$$ $\psi(x)$ depends on the value of $x$ taken by $X$, so it can be thought of as a function $\psi(X)$ of $X$ itself. $$\psi(X)= E(Y|X)$$ is called the conditional expectation of $Y$ given $X$.
:::
...
![Given each value of X, there is a distribution of Y\|X. E(Y\|X) is a function of X.](img/condist.png){fig-align="center" width="100%"}
