...
reasonably high probability. Standard choices are $\alpha=0.05$ or
$0.10$, corresponding to 95% and 90% confidence levels.

### Confidence interval of sample mean {.unnumbered}
...
in the estimates. Instead, we construct the confidence interval of the
test score difference. (We construct the CIs using $t$-scores assuming normal
distribution in scores, or use $z$-scores if the sample size is large.)

```{r, warning=}
#| warning: false
# load dataset
data <- read.csv("../dataset/exam.csv")

# test scores of male and female students
y1 <- subset(data, gender == 'Male'  )$overall 
y2 <- subset(data, gender == 'Female')$overall

# compare the means
cat("Male:", mean(y1), "Female:", mean(y2))

# t-test over d = y1 - y2
d <- t.test(y1, y2, conf.level = .95)

# CI contains 0, meaning there is no significant difference
cat("Diff:",  d$estimate, "CI: [", d$conf.int, "]")
```
