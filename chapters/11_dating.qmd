...
the previous ones. The overall probability is: 
$$\begin{aligned}
P(M,N) = 
& P(\text{Success | X is in position 1}) P(\text{X is in position 1}) + \\
& P(\text{Success | X is in position 2}) P(\text{X is in position 2}) + \\
& \quad\vdots \\
& P(\text{Success | X in position N})P(\text{X is in position N})
\end{aligned}\\ $$
