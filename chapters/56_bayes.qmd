...
Bayesians take a different approach to statistical inference. 
Bayesians treat the unknown parameters as random variables associated with 
distributions. Instead of trying to estimate the "true" value of the 
parameters, the distribution of the parameters gets updated with the 
...
is a function of $\theta$, the denominator behaves like a normalizing 
...
$$p(\mu | \boldsymbol{x}) \propto \exp\left( -\frac{1}{2} \left[ \frac{(\mu - \mu_0)^2}{v_0^2} + \frac{\sum_{i=1}^n (x_i - \mu)^2}{\sigma^2} \right] \right)$$
...
| **Probability Meaning** | **Long-Run Frequency.**<br>The frequency of occurrence out of infinitely many trials. | **Degree of Belief.**<br>How certain we are about something given our knowledge. |
